...
def user_login(request) :
    if request.method == 'POST' :
        ...
        if form.is_valid():
            ...
            if user is not None :
                login(request, user)
                messages.success(request , 'ورود کاربر با موفقیت انجام شد', 'primary')
                return redirect('home:home')
            else:
                messages.success(request, 'ورود کاربر با موفقیت انجام نشد', 'danger')
    else :
        form = UserLoginForm()
...
def user_update(request) :
    if request.method == 'POST' :
        ...
        if user_form and profile_form.is_valid():
            user_form.save()
            profile_form.save()
            messages.success(request , 'اطلاعات با موفقیت ثبت گردید', 'success')
